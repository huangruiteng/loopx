...
| Goal channel projection | `4b5b46c` defined `goal_channel_projection_v0`; `6d37e3f` added the compact projection builder; `38bb58c` rendered it into static read-only frontstage HTML; the status feed now exports the same projection on `attention_queue.items[].goal_channel_projection`. | Next slices should teach the React dashboard to render the projection as channel cards/timelines without making the browser view writable truth. |
...
| GH-C49 | dashboard | Render `goal_channel_projection_v0` in the React dashboard from `attention_queue.items[].goal_channel_projection`, reusing the frontstage fixture semantics and keeping write authority outside the channel view. | `python3 examples/goal-channel-status-export-smoke.py`, `python3 examples/goal-channel-frontstage-fixture-smoke.py`, and a focused dashboard smoke |
