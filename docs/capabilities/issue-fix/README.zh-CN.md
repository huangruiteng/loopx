...
不记录本地 repo path，且 `reviewer-plan` 本身不会发送 review request。
`reviewer-request` 会读取 live PR author、已有 review request 和已完成 review，自动排除
这些身份，邀请剩余候选中排名最高且可请求的人，并再次读取 PR 后才报告成功。History
固定读取 base revision，避免 feature branch 自己的提交把作者推荐回来；
...
执行当前 boundary authority 覆盖的动作。Review request 同样是外部写，但持续生效的
`external_review_request` 或 `publish` authority 允许 agent 不再重复询问用户，直接完成
这个例行动作；recommendation packet 始终保持只读。
...
| Reviewer request | `loopx issue-fix reviewer-request` | 在持续 authority 下自动排除 live PR author 和已有 reviewer，邀请排名最高的可请求候选，并回读验证 provider state。 |
...
- 带 authority、幂等和 provider 回读验证的 reviewer 自动邀请；
- PR lifecycle projection；
- host agent 驱动的 LoopX todo/quota/monitor/Kanban 集成。

### 下一阶段

- 把 reviewer-request transition 接入 PR-ready 和 lifecycle domain state；
...
# 在持续 authority 下邀请默认的 top non-author reviewer，并回读验证。
loopx issue-fix reviewer-request \
