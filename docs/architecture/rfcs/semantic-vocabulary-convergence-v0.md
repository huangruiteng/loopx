...
| --- | --- | --- | --- |
| Registry and inventory match the code at baseline | `uv run python examples/semantic-vocabulary-drift-smoke.py` | `ok` with coverage, ratchet, budget, and twin report | Proves parity for registered vocabularies and mapped carriers only |
...
project's Python (`>=3.11` in `pyproject.toml`), not an executable named
`python3.11`. Run from the checkout with `uv run python <script>` or
`uv run --extra test python -m pytest <tests>`; `uv run --extra test loopx
canary premerge --from-git-diff` also resolves LoopX from this checkout.
uv prepares the project environment and, by default, downloads a compatible
Python when none is installed. This requires uv and network access or a cached
interpreter; see the [uv Python documentation](https://docs.astral.sh/uv/concepts/python-versions/).
