...
| Antigravity CLI (agy) | Install the skill facade with `loopx slash-commands --install --surface agy`, then invoke the `loopx` skill (or `/loopx <complex task>`) from an `agy` session in the project. | The `agy` session's own turn loop, with the native `schedule` tool arming self-wakes while the session lives; every turn and wake enters through `quota should-run` |
