...
def check_inventory(registry: dict[str, Any], sources: list[SourceFile]) -> tuple[dict[str, Any], str]:
    ...
    require(render_inventory(inventory) == committed, f"{registry['inventory']} is stale; run uv run python scripts/generate_semantic_inventory.py and commit the result")
