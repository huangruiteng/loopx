...
def test_terminal_event_history_does_not_remain_in_the_hot_cache(tmp_path: Path) -> None:
...
def test_completed_event_compaction_is_skipped_until_the_file_changes(
    ...
    assert turn["event_compaction_revision"] == list(first._event_revision(event_path) or ())
...
def test_completed_event_compaction_marker_is_invalidated_by_a_new_event(
    ...
    with event_path.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps({"kind": "answer.delta", "sequence": 3, "event_id": "3"}))
        handle.write("\n")
...
def test_missing_event_stream_is_marked_without_retaining_empty_state(tmp_path: Path) -> None:
