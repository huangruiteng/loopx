...
export function GoalOverview({ active, goal, items, userTodos, readOnly, onOpenDetails, onSelect, onView }: {
  ...
  const copy = locale === "zh-CN" ? {
    ...
    tasks: "查看任务", outputs: "查看成果", usage: "最近 24 小时", execution: "执行记录",
    ...
    tasks: "View tasks", outputs: "View outputs", usage: "Last 24 hours", execution: "Execution",
  ...
  return <section className="goal-overview" aria-label={t("header.overview")}>
    ...
    <div className="goal-overview-summary">
      <section><header><h3>{copy.progress}</h3><span>{localizedGoalState(goal.state, locale)}</span></header>
        <strong>{goal.nextSentence}</strong><p>{goal.agentSentence}</p>
        <div className="goal-overview-links"><button type="button" onClick={() => onView("tasks")}>{copy.tasks}<ArrowRight size={14} /></button><button type="button" onClick={() => onView("files")}>{copy.outputs}<ArrowRight size={14} /></button></div>
      ...
      <section><header><h3>{copy.attention}</h3><span>{attention.length}</span></header>
        {attention.length ? <ul>{attention.slice(0, 3).map(item => <li key={item.todoId}><button type="button" onClick={() => onSelect({ kind: "attention", item })}><span>{item.text}</span><ArrowRight size={15} /></button></li>)}</ul> : <p>{copy.none}</p>}
        {attention.length > 3 ? <button type="button" onClick={() => onView("tasks")}>{copy.tasks} ({attention.length})<ArrowRight size={14} /></button> : null}
      </section>
    </div>
    <dl className="goal-overview-usage" aria-label={copy.usage}>
      <div><dt>{t("drawer.tokensShort")} · {copy.usage}</dt><dd>{formatUsageValue(goal.usage?.tokens24h, t("drawer.usageNotMeasured"), formatTokenCount)}</dd></div>
      <div><dt>{t("drawer.costShort")} · {copy.usage}</dt><dd>{formatUsageValue(goal.usage?.costUsd24h, t("drawer.usageNotMeasured"), formatCostUsd)}</dd></div>
      <div><dt>{t("drawer.durationShort")} · {copy.usage}</dt><dd>{formatUsageValue(goal.usage?.durationMs24h, t("drawer.usageNotMeasured"), formatDurationMs)}</dd></div>
    </dl>
