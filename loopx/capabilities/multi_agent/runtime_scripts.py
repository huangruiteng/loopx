...
prompt_target.write_text(
    ...
    "            '- 若 tick 没有选中 todo，先用 `$LOOPX_PANE_LOOPX todo list --goal-id \"$LOOPX_GOAL_ID\"` 和 status 找本角色 open todo；不要一上来 diagnose。',\n"
    ...
    "            '- If the tick has no selected todo, first use `$LOOPX_PANE_LOOPX todo list --goal-id \"$LOOPX_GOAL_ID\"` and status to find this role open todo; do not start with diagnose.',\n"
...
tick_target.write_text(
    ...
    "        round_record['appended_count'] = append.get('appended_count')\n"
    "    if payload.get('successor_todos'):\n"
    "        round_record['successor_todo_declared'] = True\n"
    ...
    "        round_record['holdout_improvement_count'] = evaluation.get('holdout_improvement_count')\n"
    ...
    "    round_records.append({'round_index': 1, 'quota_status': status, 'frontier_status': frontier_status, 'worker_configured': False, 'worker_executed': False, 'worker_status': None})\n"
    "    ok = status == 0 and frontier_status == 0\n"
    "    write_rounds_summary(ok=ok, status='no_worker_command_configured' if ok else 'frontier_failed' if status == 0 else 'quota_failed', rounds_requested=1)\n"
    "    if status != 0:\n"
